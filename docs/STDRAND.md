...
Acesso às instruções de entropia da Intel/AMD (`RDRAND`/`RDSEED`). As funções possuem proteções internas (`CPUID`) para evitar _crashes_ em CPUs antigas que não suportam estas instruções.
